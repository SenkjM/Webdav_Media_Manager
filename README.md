...
- **本地音乐库**：仅索引「至少缓存过一次」的曲目；可按 **专辑 / 作者 / 音乐名 / 标签（流派）** 浏览；支持 **搜索**（标题/艺术家/专辑）；长按进入多选（添加到歌单 / 分享 / 删除，CUE 组删除仍整组警告）；支持按 **名称** 或 **专辑曲序（碟号/曲号）** 排序；身份键为 `(webdav_account_id + remote_path)`
...
- **CUE 分享裁剪**：分享 CUE 虚拟曲目时用 **FFmpeg**（`ffmpeg_kit_flutter_new_audio`；桌面/测试回退系统 `ffmpeg`）按 `clipStart/clipEnd` 导出独立可播片段，并写入 title/artist/album/track 等标签；非 CUE 曲目直接分享本地缓存文件（`share_plus`）
...
**全局迷你播放条**：挂在应用壳（`HomeShell`）上，音乐库 / 歌单 / 网络库 / 下载队列及其子页（嵌套 Navigator）均可见；**设置页隐藏**。正在下载待播曲目时，迷你条显示该曲下载进度（不可当作已可完整播放）。
