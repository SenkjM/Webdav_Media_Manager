...
- 销毁：逐首立墓碑 → 清空本地库 → 定时同步置为 `off`。
- 两个动作期间不允许后台写库。

**取舍**：

- 覆盖只清**索引**（新增 `LibraryDatabase.clearLibraryIndex()`），不清 cache annex：连 annex 一起清的话，磁盘上已存在的音频会被当成没下过，白白多出一次全量下载。
- 销毁顺手修掉了原先的墓碑缺失：整库销毁原本走 `clearAllLibraryData()`，会把 `deleted_tracks` 整表清空，于是同步一条删除记录都拿不到。现在先 `recordTombstone()` 再清表。
- 关掉定时同步是刻意的，不是顺手：空索引 + 空游标下第一趟自动同步会把整库拉回来，等于把用户刚做的销毁撤销掉。
