...
- 销毁：对全库跑一遍逐条销毁（每首：墓碑 → 缓存音频 → 封面 → 库行 → 内存），最后统一通知界面，再把定时同步置为 `off`。
- 两个动作期间不允许后台写库。
...
- 与曲库页那条逐条销毁按钮的差别只有两处：不清歌单里的悬空引用（老实现里的 `playlists.removeEntriesNotIn({})`）、不动下载队列（老的 `downloads.invalidateMissingCompleted()`）。要补是两行的事。
