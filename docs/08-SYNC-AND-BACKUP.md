...
- 音乐库区块除「同步 / 重建 / 整理」外，另有上下两个**红底白字的不可逆动作**。两者都要二次确认：确认框正文说会发生什么，小一号字补后果，最后**手打 `YES`（不区分大小写）**确认按钮才可用。两个动作是一对反义词——一个把云端拉到本地，一个把本地的删除推到云端。
  ...
  - **销毁音乐库**：对全库跑一遍「逐条销毁」——每首先立墓碑，再删缓存音频、封面与库记录，最后统一通知界面，曲目随即从本地曲库消失。删除记录作为 `del-*.wdmm` 在下次同步时推上云端，云端在下一次重建时才真正抹掉这些行。结束时把「定时同步」设为关闭，什么时候推由用户手动决定。与曲库页那条逐条销毁按钮相比只少两件事：不清歌单里的悬空引用，也不动下载队列（详见 [99 §3](99-IN-PROGRESS.md)）。
