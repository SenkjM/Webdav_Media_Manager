...
### 6.1 凭证同步/备份的加密范围（新驱动必读）

- **加密范围 = `spec.secretFieldKeys`，自动推导，不用手动登记**：表单声明 `obscure: true` 的字段（「配置界面默认为密码的数据」）∪ `spec.runtimeSecretKeys`（运行时令牌缓存键）。凭证库（`credentials.json` v2）与备份归档都按这个集合逐字段 AESGCMv1 加密；范围外的字段明文进档案。
- **唯一的例外是 `runtimeSecretKeys`，必须人工声明**：驱动在运行时经 `onTokenUpdate` 写回驱动配置的令牌缓存（如百度 / 123 的 `access_token`）**不在表单里**，obscure 声明看不见它——落地驱动时要对照客户端代码里所有 `onTokenUpdate?.call({...})` 的键，把凭证类键补进 `runtimeSecretKeys`，否则令牌**明文**进凭证库与备份。没有令牌轮换的驱动（netease / crypt）保持默认空集。
- **恢复侧同样自动**：条目的 `providerType` 在注册表查不到 → 静默跳过（08 §2）；按 AESGCMv1 前缀自识别密文、逐字段解密，驱动无需参与。
...
除本节上面的三条外，新驱动落地时按 [§6.1](#61-凭证同步备份的加密范围新驱动必读) 过一遍凭证链路：
