...
README 的[后续计划（路线图）](../README.md#后续计划路线图)表是对外可见的唯一计划概览；本节补充 coding agent 需要的上下文。**本表仅规划，未排期实现；除非用户明确要求，agents 不得开始这些工作，也不要把规划项当作已实现功能。**

- **WebDAV 管理增强**：现有网络库已经支持新建文件夹、重命名、删除；后续若获授权，应在此基础上扩展上传本地文件、移动/复制、属性查看、批量操作、回收站与覆盖冲突策略。主要入口和相关服务是 `lib/screens/network_library_screen.dart`、`lib/services/webdav_service.dart`、`lib/screens/accounts_screen.dart`，先核对现有 WebDAV 能力与账号/权限处理，再设计交互。
- **媒体与视频**：ColorOS / 一加等国产 ROM 的媒体通知、后台保活仍需真机验证与加固；视频缓冲第二层进度条、空闲隐藏控件、手势引导对应 `docs/DEVICE-QA-CHECKLIST.md` 的未做项。
- **同步与播放**：同步冲突可视化、分片整理体验，以及 AudioTrack / AAudio 等可选音乐输出后端都只是规划；后者成本较高，不应在未获授权时启动。
- **跨平台与体验**：iOS / 桌面端是 Android-first 前提下的远期方向；无障碍、多语言和启动/低端机体验也需按路线图排期。
