...
> **文档编号 D01** · 状态：进行中 · 模式：活跃（可继续维护）  
> 总索引：[INDEX.md](../INDEX.md)

### 文档体系规则（Agents）

- 实现功能前先读 [INDEX.md](../INDEX.md) 与状态为「进行中」的计划文档（当前见 D02）。
- **不得修改** INDEX 中状态为「已完成 / 归档」的文档正文；勘误请新建编号文档或勘误条，并在 INDEX 备注。
- 新发规划 / 变更文档：取下一个编号、写入 INDEX、状态「进行中」、放入 `active/`。
- 完成后：INDEX 标「已完成」→ 移入 `archive/` → 模式「归档」。
- 本文件（D01）为常驻活文档，保持活跃；日常约束与分支策略以本文为准。
...
下次开发分块计划（后台下载优先 + 文件动作 / 多选重构）见：[D02-NEXT-DEV-PLAN.md](D02-NEXT-DEV-PLAN.md)（唯一计划源）。文档总索引：[INDEX.md](../INDEX.md)。
...
| 中 | 视频播放体验 | 缓冲进度条第二层、空闲自动隐藏控件、手势引导；见 `D03-DEVICE-QA-CHECKLIST` 未做项。 | 规划中 |
