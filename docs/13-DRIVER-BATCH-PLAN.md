...
- 真机：**验收通过（用户确认）**——123_open 添加与浏览在真机走通；打开目录时出现过一次 invalid_grant（在线续期返回的令牌失效类错误），**用户决定不排查、按验收通过收口**，现象留档于 99 §4.3.4。其余三盘一并按通过处理。原清单留档：按 [12 §9](12-DRIVER-PORTING-GUIDE.md) 逐盘过；
  **A 类（有测试条件）先验，其余靠发布后用户反馈**（[99 §4.3](99-IN-PROGRESS.md) 的先例）。

### 6.1 各盘验收要点（真机验收已按通过处理；下表留档备查）
