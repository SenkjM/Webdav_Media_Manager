...
3. **分支**：`main` 是唯一主干，只有用户明确允许才可合并。新功能一律**基于 `main` 开独立功能分支**，成熟后**压缩成单个提交**合入 `main`（除非用户另有要求，例如要保留每一步的提交历史）。**压缩时要重写 commit 名称与描述**，不要沿用分支上某一条的信息、也不要罗列原始提交：按整条分支实际做了什么重新组织，一条读完就知道改了什么、为什么改。不开长期保存线，中途成果留在自己的功能分支上。使用worktree进行分支工作，worktree建立在/worktree下面。
