...
- **自动连播**：控制器（`VideoQueueController`）不实现推进，谁开播谁接下一首——这一页监听 `player.stream.completed`。
- **播放模式**（控制条最左边轮换）：单曲循环 / 顺序 / 列表循环。存在设置里，下次进页面沿用。**顺序**到队尾停住，**列表循环**回第一首。
- **播放列表**（控制条最右边）：面板外形与视频页一致，另加**搜索框**；搜索激活时每行多一行相对目录，用来区分子目录里的同名曲。
- **搜索子目录**由「设置 → 音频流式」的开关决定：关掉就只列当前这一层。
